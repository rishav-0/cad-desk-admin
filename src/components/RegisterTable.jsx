...
const RegisterTable = ({ allforms }) => {
  ...
  const saveRemarks = async () => {
    ...
    try {
      await updateDoc(doc(db, "register", userId), {
        status: "remarks",
        remarks,
      });
      ...
      setFormlist((prev) =>
        prev.map((form) =>
          form.id === userId ? { ...form, status: "remarks", remarks } : form
        )
      );
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    if (newStatus === "remarks") {
      const currentForm = formlist.find((form) => form.id === id);
      openRemarksDialog(id, currentForm?.remarks || "");
      return;
    }
    ...
    try {
      await updateDoc(doc(db, "register", id), {
        status: newStatus,
        remarks: "",
      });
                ...
                {formlist.map((form) => (
                  <React.Fragment key={form.id}>
                    <tr
                      ...
                      onClick={() => toggleDescription(form.id)}
                    >
                      ...
                      <td colSpan="5" className="p-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <p>
                              {form.courseUpgrade?.nameOfCourse?.join(", ")}
                            </p>
                          </div>
                          <div>
                            <p className="font-semibold">Software Covered:</p>
                            <p>
                              {form.courseUpgrade?.softwareCovered?.join(", ")}
                            </p>
                          </div>
                          <div>
                            <p className="font-semibold">Course Fee:</p>
                            <p>{form.courseUpgrade?.courseFee}</p>
                          </div>
                          <div>
                            <p className="font-semibold">Joining Date:</p>
                            <p>{form.courseUpgrade?.joiningDate}</p>
                          </div>
                          <div>
                            <p className="font-semibold">Qualification:</p>
                            <p>{form.personalDetail?.qualification}</p>
                          </div>
                          <div>
                            <p className="font-semibold">College:</p>
                            <p>{form.personalDetail?.college}</p>
                          </div>
                          <div>
                            <p className="font-semibold">Father's Name:</p>
                            <p>{form.personalDetail?.fathersName}</p>
                          </div>
                          <div>
                            <p className="font-semibold">Address:</p>
                            <p>{form.personalDetail?.contractAddress}</p>
                          </div>
                          <div>
                            <p className="font-semibold">Date of Birth:</p>
                            <p>{form.personalDetail?.Dob}</p>
                          </div>
                          <div>
